...
/**
 * Callback ref: يُستدعى من React في اللحظة التي يُركَّب فيها العنصر التفاعلي
 * (Pressable → div على web) وأيضاً عند فصله. هذا يحل ثغرة التوقيت في useEffect:
 *   - مع useRef + useEffect([]) كان الـ effect يُنفذ مرة واحدة قبل ركوب الزر،
 *     فلا يُربط أي listener أبداً عند فتح المحادثة لأول مرة.
 *   - مع callback ref الربط يحدث بالضبط عند ظهور الـ node في الـ DOM،
 *     ويتم تنظيفه بالضبط عند اختفائه.
 *
 * الهدف: منع المتصفح من نقل الـ focus من الـ TextInput إلى الزر عند الضغط
 * (mousedown/pointerdown)، فيبقى الـ input مركّزاً والكيبورد مفتوحاً.
 */
function useKeepInputFocusOnWeb() {
  ...
  return useCallback((node: View | null) => {
    ...
    if (previousNode && previousListener) {
      previousNode.removeEventListener("mousedown", previousListener);
      previousNode.removeEventListener("pointerdown", previousListener);
    }

    nodeRef.current = null;
    listenerRef.current = null;

    if (!IS_WEB || !node) return;
    ...
    nodeRef.current = domNode;
    listenerRef.current = listener;
    domNode.addEventListener("mousedown", listener);
    domNode.addEventListener("pointerdown", listener);
  }, []);
}
        ...
        <View style={styles.composer}>
          ...
          <View style={[styles.inputRow, isFocused && styles.inputRowFocused]}>
            <Pressable
              ...
              onPress={() => {
                // صمام أمان: استعادة التركيز فوراً داخل user gesture
                // قبل بدء أي عملية async — هذا يضمن بقاء الكيبورد مفتوحاً
                // حتى لو نجح المتصفح في سحب الـ focus للحظة.
